...
function exo_extract_remote_zip($new_file_loc, $tmp_file_loc, $zip_url) {
    ...
    // read the zip
    exo_download_in_chunks($zip_url, $tmp_file_loc, $chunkSize = 1, $returnbytes = true);
    ...
    if ($res === TRUE) {
        ...
        if(! $zip->extractTo($new_file_loc)){
            echo 'Couldnt extract to '.$new_file_loc.'!<br>';
        }
        $zip->close();
        echo 'Deleting local copy '.$tmp_file_loc.'....<br>';
        unlink($tmp_file_loc);
        return 1;
        echo 'Extracted '.$new_file_loc.'..<br>';


    } else {
        echo 'Failed to open tmp zip at '.$tmp_file_loc.'!<br>';
        return 0;
    }
}
...
function exo_download_in_chunks($srcName, $dstName, $chunkSize = 1, $returnbytes = true) {
    ...
    if ($returnbytes && $status) {
        echo 'Downloaded '.$bytesCount.' Bytes for '.$dstName.'<br>';
